...
public final class TweakedControllerCableServerHandler {
    ...
    public static void tick(final Level level) {
        tickTimeouts(level, BUTTON_TIMEOUTS.get(level), BUTTON_TO_CHANNEL, false);
        tickTimeouts(level, AXIS_TIMEOUTS.get(level), AXIS_TO_CHANNEL, true);
    }

    public static void receiveAxis(
            final Level level,
            final BlockPos pos,
            final List<Byte> axisStates,
            final int axis
    ) {
        receiveAxisStates(level, pos, axisStates);
        if (level.getBlockEntity(pos) instanceof final AdvancedCableHubBlockEntity hub) {
            hub.receiveAxisStates(axis);
        }
    }

    public static void receiveFullAxis(
            final Level level,
            final BlockPos pos,
            final List<Byte> axisStates,
            final float[] fullAxis
    ) {
        receiveAxisStates(level, pos, axisStates);
        if (level.getBlockEntity(pos) instanceof final AdvancedCableHubBlockEntity hub) {
            hub.receiveFullAxisStates(fullAxis);
        }
    }

    private static void receiveAxisStates(final Level level, final BlockPos pos, final List<Byte> axisStates) {
    ...
    public static void receiveButtons(final Level level, final BlockPos pos, final List<Boolean> buttonStates) {
        ...
        if (level.getBlockEntity(pos) instanceof final AdvancedCableHubBlockEntity hub) {
            hub.receiveButtons(buttonStates);
        }
    }
    ...
    private static void tickTimeouts(
        ...
        while (iterator.hasNext()) {
            ...
            if (ttl <= 0) {
                ...
                if (level.getBlockEntity(key.getFirst()) instanceof final AdvancedCableHubBlockEntity hub) {
                    if (axis) {
                        hub.clearAxis(key.getSecond());
                    } else {
                        hub.clearButton(key.getSecond());
                    }
                }
                iterator.remove();
                continue;
